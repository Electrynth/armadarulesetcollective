    ...
    <nav className="fixed top-0 left-0 right-0 z-50 font-montserrat py-3">
      <div className="max-w-6xl mx-auto px-4">
        <div className="flex flex-col md:flex-row justify-center items-center relative">
          {/* Hamburger/X menu button - always visible on mobile */}
          <button 
            ref={buttonRef}
            className="md:hidden text-white p-2 absolute right-4 top-0 z-[60]"
            onClick={toggleMenu}
          ...
          <div className="hidden md:flex bg-gray-800/90 backdrop-blur-sm rounded-full px-6 py-2 ring-1 ring-gray-700/50">
            {navLinks.map((link) => (
              <Link
                key={link.path}
                to={link.path}
                className={`text-white hover:text-[#C14949] transition-colors px-4 py-1 ${isActive(link.path)}`}
              >
                {link.label}
              </Link>
            ))}
          ...
          <div className="flex flex-col bg-gray-800/90 backdrop-blur-sm rounded-lg p-3 ring-1 ring-gray-700/50 text-left">
            {navLinks.map((link, index) => (
              <div key={link.path}>
                <Link
                  to={link.path}
                  className={`text-white hover:text-[#C14949] transition-colors block py-1.5 text-left ${isActive(link.path)}`}
                >
                  {link.label}
                </Link>
